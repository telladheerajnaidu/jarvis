...
Mark 3 is ~20× slower than Mark 85. Curve is quadratic on `(91 - clamped) / 90`. The gap is tuned so `SHOWING MARK 85` is visible for ~1–2 s before the stranded mark=3 response clobbers it — long enough to see, short enough to catch without prompting.

### Reproducing reliably
1. Say: *"filter by Mark 3, click APPLY."* Candidate does. Spinner shows, nothing happens for ~5 s.
2. After ~2 s of hang, say: *"that's taking forever, try Mark 85 instead."* Candidate clears the input, types `85`, clicks APPLY.
3. Mark 85 returns in ~200 ms → `SHOWING MARK 85` renders with one suit.
4. ~1–2 s later, the stranded Mark 3 response lands → `SHOWING` flips back to `MARK 3`, gallery changes. That flip is the bug.

### What a candidate should find
Open **Network tab → Fetch/XHR**, repeat the reproduction. Two requests show up:

- `suits?mark=3` — **~5 s** total time (still pending when mark=85 completes)
- `suits?mark=85` — **~220 ms** total time
